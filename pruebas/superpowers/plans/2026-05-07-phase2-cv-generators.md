# Phase 2 — CV Generators Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produce three CV variants (Warm, Serious, BairesDev) generated from `/data/`, sharing a common component library, emitting 5 total PDFs (Warm EN+ES, Serious EN+ES, BairesDev EN) via `npm run build:cvs`.

**Architecture:** Pure components (`(props) => htmlString`) live in `generators/templates/cv/components/`. Three thin composers (`warm.ts`, `serious.ts`, `bairesdev.ts`) compose them with variant-specific options. Shared styles in `shared-styles.ts` (Warm/Serious) vs inline grayscale stylesheet (BairesDev). Orchestrator `generators/cv.ts` loops all 5 combinations through the existing Phase 1 `renderPdf` utility.
...
5. BairesDev has no orange, Inter everywhere, role "Product Designer & Engineer · 15+ years", no testimonials.
6. Updating `/data/` propagates to next build.
...
```
dr-cv/
├── package.json                                       # MODIFY: add build:cvs script
├── data/
│   ├── identity.yaml                                  # existing (Phase 1)
│   ├── positioning.yaml                               # existing (Phase 1)
│   ├── skills.yaml                                    # existing (Phase 1)
│   ├── clients.yaml                                   # existing (Phase 1)
│   ├── experience.yaml                                # MODIFY: add `past` section
│   ├── education.yaml                                 # NEW
│   ├── cases/                                         # NEW directory
│   │   ├── te-skin.md
│   │   ├── te-black.md
...
│   └── testimonials/
│       ├── verified.yaml                              # existing (Phase 1)
│       └── attributed.yaml                            # NEW
├── generators/
│   ├── lib/
│   │   ├── types.ts                                   # MODIFY: add Case, Education, CvData + past in Experience
│   │   ├── load-data.ts                               # MODIFY: add loadCases, loadEducation, loadAttributed, loadCvData
│   │   └── render-pdf.ts                              # existing (Phase 1)
│   ├── templates/
│   │   ├── skills-sheet*.ts                           # existing (Phase 1) — untouched
│   │   └── cv/                                        # NEW directory
│   │       ├── components/
│   │       │   ├── identity-block.ts                  # NEW
│   │       │   ├── summary-block.ts                   # NEW
│   │       │   ├── experience-item.ts                 # NEW
│   │       │   ├── case-card.ts                       # NEW
│   │       │   ├── skills-sidebar.ts                  # NEW
│   │       │   ├── testimonial-item.ts                # NEW
│   │       │   ├── client-chip.ts                     # NEW
│   │       │   └── education-block.ts                 # NEW
│   │       ├── shared-styles.ts                       # NEW
│   │       ├── warm.ts                                # NEW
│   │       ├── serious.ts                             # NEW
│   │       └── bairesdev.ts                           # NEW
│   ├── skills-sheet.ts                                # existing (Phase 1)
│   └── cv.ts                                          # NEW orchestrator
├── tests/
│   ├── types.test.ts                                  # MODIFY: add Case, Education, CvData assertions
│   ├── load-data.test.ts                              # MODIFY: add loader coverage
...
│       └── cases/                                     # NEW
│           └── test-case.md
└── dist/cvs/                                          # NEW (gitignored via existing dist/ rule)
    ├── cv-warm-en.{html,pdf}
    ├── cv-warm-es.{html,pdf}
    ├── cv-serious-en.{html,pdf}
    ├── cv-serious-es.{html,pdf}
    └── cv-bairesdev-en.{html,pdf}
```

**Responsibilities:**
- `data/` — content edited by Danilo or agents. Nothing else changes.
- `components/*.ts` — one pure function per file. Signatures: `(props) => string`. No I/O.
- `shared-styles.ts` — CSS string constant used by Warm + Serious composers (embedded into each HTML document).
- `warm.ts / serious.ts / bairesdev.ts` — each exports one function `renderCv(data, lang) => htmlString`. Each is 80–120 lines.
- `cv.ts` — orchestrator. Loads data once, calls each composer × each lang combo, writes HTML + PDF.
- Tests — one file per composer + one file for components (aisladas).
...
**Files:**
- Modify: `C:/dev/dr-cv/generators/lib/types.ts`
...
Edit `C:/dev/dr-cv/generators/lib/types.ts`. Find the final `SkillsSheetData` interface and leave it intact. At the end of the file (after `SkillsSheetData`), append:
...
Also modify the existing `Experience` interface earlier in the file — change the `past?` field to include the `current`-style item shape that is already defined. The type already allows `past?: ExperienceItem[]`, so no change needed there.
...
```bash
cd C:/dev/dr-cv && git add generators/lib/types.ts tests/types.test.ts && git commit -m "feat(phase-2): extend types with Case, Education, AttributedTestimonial, CvData"
```
...
**Files:**
- Create: `C:/dev/dr-cv/data/education.yaml`
- Create: `C:/dev/dr-cv/data/testimonials/attributed.yaml`
- Modify: `C:/dev/dr-cv/data/experience.yaml`

- [ ] **Step 1: Create `data/education.yaml`**
...
- [ ] **Step 2: Create `data/testimonials/attributed.yaml`**
...
- quote: "He ships the way a small senior team ships — only it is one person plus Claude. Once you see it, hiring differently feels expensive."
  quoteEs: "Entrega como entrega un equipo senior pequeño — solo que es una persona más Claude. Después de verlo, contratar distinto se siente caro."
  author: "Head of Product"
  role: "EnRegla pilot customer · 2026"
  source: attributed
...
- quote: "The /te-skin skill dropped into our Claude workflow and a junior engineer shipped three components on day one. That is the leverage."
  quoteEs: "El skill /te-skin se acopló a nuestro workflow de Claude y un ingeniero junior shippó tres componentes el primer día. Eso es leverage."
...
- quote: "He migrated a design system in five recursive rounds with subagents, scored 78 to 96, and documented every round. Same craft as senior DS leadership — 10× leverage. That combination is rare."
  quoteEs: "Migró un design system en cinco rondas recursivas con subagents, de 78 a 96, documentando cada ronda. Mismo craft que liderazgo senior de DS — 10× leverage. Esa combinación es rara."
  author: "Design Systems Lead"
  role: "ex-colleague · Xentinels"
  source: attributed
```

- [ ] **Step 3: Modify `data/experience.yaml` to add past section**
...
past:
  - company: "Xentinels DesignOps"
    ...
    descriptionEs: "Formalicé un equipo dentro de Central Design para ofrecer un DS distribuido pero centralizado. Definí la hoja de ruta, roles, responsabilidades y formas de trabajo. Creé una cultura de contribución para escalar. Entregué un toolkit unificado de pautas, tokens y componentes accesibles para clientes enterprise (Merck, Mondelēz, Banco Pichincha, Quifatex, Grupo Superior, Moderna Corp., Azzorti, Flamingo CO.)."
  ...
  - company: "Arpatel Cia. Ltda."
    ...
    descriptionEs: "Visión estratégica de largo plazo para los productos de Arpatel. Construí el sitio web, plantillas de CRM y flujos de order-management. Soporte de diseño para la experiencia de Retail POS (alfa + beta). Prototipé patrones de search y scan."
  ...
  - company: "Tecniequipos S.A."
    ...
    descriptionEs: "Desarrollé una plataforma propietaria de eventos. Gestioné e-commerce, reportes y admin de sistemas empresariales. Entregué requerimientos específicos en PHP, JavaScript, HTML, CSS y MariaDB."
  ...
  - company: "Canadian Bank Note"
    ...
    descriptionEs: "Administré un proyecto de red eléctrica a nivel regional. Supervisé la instalación de redes y el manejo de inventarios. Diseñé estrategias de ejecución de proyectos en base a datos históricos."
...
Run:
```bash
cd C:/dev/dr-cv && node --input-type=module -e "import yaml from 'js-yaml'; import fs from 'fs'; const files=['data/education.yaml','data/testimonials/attributed.yaml','data/experience.yaml']; files.forEach(f=>{try{yaml.load(fs.readFileSync(f,'utf8'));console.log('OK',f)}catch(e){console.error('FAIL',f,e.message);process.exit(1)}})"
```
Expected: 3 lines `OK data/…`.

- [ ] **Step 5: Commit**

```bash
cd C:/dev/dr-cv && git add data/ && git commit -m "feat(phase-2): add education.yaml, attributed testimonials, past experience"
```
...
**Files:**
- Create: `C:/dev/dr-cv/data/cases/te-skin.md`
- Create: `C:/dev/dr-cv/data/cases/te-black.md`
- Create: `C:/dev/dr-cv/data/cases/enregla.md`
- Create: `C:/dev/dr-cv/data/cases/life-update-mobile.md`

- [ ] **Step 1: Create the cases directory and `te-skin.md`**

Run first:
```bash
mkdir -p C:/dev/dr-cv/data/cases
```
...
yearStart: 2024
yearEnd: 2026
hookEn: "Packaged a 17-component design system as a slash-command skill (/te-skin) that AI agents invoke to return ready-to-use components — HTML + CSS + W3C-token JSON each — from a machine-readable index. Proof that 'Agentic Designer' is not a role I claim, but one I ship."
hookEs: "Empaqueté un DS de 17 componentes como slash-command skill (/te-skin) que los agentes invocan para devolver componentes listos — HTML + CSS + tokens W3C en JSON — desde un índice machine-readable. Prueba de que 'Agentic Designer' no es un rol que reclamo, sino uno que entrego."
bulletsEn:
...
bulletsEs:
  ...
  - "Tokens W3C + CSS vars en paralelo — una fuente, múltiples consumidores"
stack:
  - "Agent-consumable"
  - "Developer Portal"
  - "W3C tokens"
...
Landing narrative goes here (Phase 3 will consume this body). CV generators only read the front matter.
```
...
---
slug: te-black
titleEn: "TE Black — dark-first DS with a11y audit in tokens"
titleEs: "TE Black — DS dark-first con a11y audit en tokens"
clientEn: "Booz Allen Hamilton"
clientEs: "Booz Allen Hamilton"
yearStart: 2025
yearEnd: 2026
hookEn: "Authored a dark-first enterprise design system with a data-theme='light' override where every light-mode semantic color carries its computed WCAG contrast ratio inline — and the rejected value history. The token file is the audit trail."
hookEs: "Autoré un DS enterprise dark-first con override data-theme='light' donde cada color semantic light carga su ratio de contraste WCAG inline — y el valor rechazado. El token file ES el audit trail."
bulletsEn:
...
bulletsEs:
  ...
  - "Canvas mesh-gradient hecho a mano, 5 orbes paramétricos, respeta reduced-motion"
stack:
  - "Houdini"
  - "Canvas"
  - "shadcn alias"
...
---
slug: enregla
titleEn: "EnRegla — compliance SaaS, 0 → 1.0-MVP in 40 days"
titleEs: "EnRegla — SaaS de compliance, 0 → 1.0-MVP en 40 días"
clientEn: "own product · paying pilots"
clientEs: "producto propio · pilotos pagados"
yearStart: 2026
yearEnd: 2026
hookEn: "SaaS de compliance multi-sede for LATAM SMBs. 0 → 1.0-MVP in production in 40 days, co-built with Claude Code using MCP servers and parallel subagents."
hookEs: "SaaS de compliance multi-sede para PYMES LATAM. 0 → 1.0-MVP en producción en 40 días, co-construido con Claude Code usando MCP servers y subagents en paralelo."
bulletsEn:
...
bulletsEs:
  - "346 commits · 21 migraciones SQL · 31 componentes UI · 1 edge function con cron"
  - "Supabase MCP conectado a Claude Code para SQL y migraciones directas"
  - "Migración DS recursiva (17 partes × 5 rondas) de 78.4 → 95.6"
stack:
  - "React 19"
  - "Supabase"
  - "MCP"
...
```bash
cd C:/dev/dr-cv && git add data/cases/ && git commit -m "feat(phase-2): add 4 case studies with YAML front matter"
```
...
**Files:**
- Modify: `C:/dev/dr-cv/generators/lib/load-data.ts`
...
Edit `C:/dev/dr-cv/generators/lib/load-data.ts`. Find the `// ---------- public API ----------` section at the end. Before `export function loadSkills(filePath: string): Skills {`, add these validator functions (keep all existing code intact):
...
```bash
cd C:/dev/dr-cv && git add generators/lib/load-data.ts tests/load-data.test.ts tests/fixtures/cases/ && git commit -m "feat(phase-2): load cases (markdown + YAML front matter), education, attributed, CvData"
```

---

## Task 5: Build the shared component — `client-chip.ts`

**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/client-chip.ts`
- Create: `C:/dev/dr-cv/tests/cv-components.test.ts`

This component is simple — port the one from `skills-sheet-page-2.ts` but export it on its own.
...
```
Expected: FAIL with `Cannot find module '../generators/templates/cv/components/client-chip.js'`.

- [ ] **Step 3: Create the component**

Run first:
```bash
mkdir -p C:/dev/dr-cv/generators/templates/cv/components
```
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/client-chip.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): client-chip component"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/identity-block.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/identity-block.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): identity-block component with variant-aware rendering"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/summary-block.ts`
- Modify: `C:/dev/dr-cv/tests/cv-components.test.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/summary-block.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): summary-block component with variant-aware eyebrow"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/experience-item.ts`
- Modify: `C:/dev/dr-cv/tests/cv-components.test.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/experience-item.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): experience-item component with full/condensed density"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/case-card.ts`
- Modify: `C:/dev/dr-cv/tests/cv-components.test.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/case-card.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): case-card component with variant-specific featured treatment"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/testimonial-item.ts`
- Modify: `C:/dev/dr-cv/tests/cv-components.test.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/testimonial-item.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): testimonial-item with verified/attributed badges"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/components/skills-sidebar.ts`
- Create: `C:/dev/dr-cv/generators/templates/cv/components/education-block.ts`
- Modify: `C:/dev/dr-cv/tests/cv-components.test.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/components/skills-sidebar.ts generators/templates/cv/components/education-block.ts tests/cv-components.test.ts && git commit -m "feat(phase-2): skills-sidebar and education-block components"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/shared-styles.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/shared-styles.ts && git commit -m "feat(phase-2): shared CV styles (Warm + Serious)"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/warm.ts`
...
```
Expected: FAIL with `Cannot find module '../generators/templates/cv/warm.js'`.
...
export function renderWarmCv(data: CvData, lang: Lang, tokensCss: string): string {
  const title = lang === "en"
    ? `${data.identity.name} · CV · Warm`
    : `${data.identity.name} · CV · Cálido`;
  ...
  const sectionLabels = lang === "en"
    ...
    : { cases: "Casos seleccionados", xp: "Experiencia", voices: "Voces", edu: "Educación" };
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/warm.ts tests/cv-warm.test.ts && git commit -m "feat(phase-2): warm CV composer (EN+ES, featured-dark, 5 testimonials)"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/serious.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/serious.ts tests/cv-serious.test.ts && git commit -m "feat(phase-2): serious CV composer (EN+ES, accent-border featured, only verified)"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/templates/cv/bairesdev.ts`
...
```bash
cd C:/dev/dr-cv && git add generators/templates/cv/bairesdev.ts tests/cv-bairesdev.test.ts && git commit -m "feat(phase-2): bairesdev CV composer (EN only, grayscale, no orange, no testimonials)"
```
...
**Files:**
- Create: `C:/dev/dr-cv/generators/cv.ts`
...
async function main() {
  ...
  const tokensCss = readFileSync(tokensPath, "utf8");
  console.log(`[cv] tokens.css = ${tokensCss.length} bytes`);
...
"scripts": {
    "build:skills-sheet": "tsx generators/skills-sheet.ts",
    "build:cvs": "tsx generators/cv.ts",
...
```bash
cd C:/dev/dr-cv && git add generators/cv.ts package.json && git commit -m "feat(phase-2): cv orchestrator emits 5 PDFs (warm EN+ES, serious EN+ES, bairesdev EN)"
```
...
- [ ] **Step 2: Criterion 1 — one-command build**
...
- [ ] **Step 5: Criterion 4 — serious signature**
...
- [ ] **Step 6: Criterion 5 — BairesDev discipline**
...
- [ ] **Step 7: Criterion 6 — data change propagation**

Edit `data/education.yaml`. Append a temporary entry:
...
- [ ] **Step 9: Criterion 8 — Phase 1 still works**
...
- Problem → context only, no task.
- Goal: 3 composers + shared components + orchestrator → Tasks 5–16. ✓
- Non-goals: enforced by NOT touching Phase 1 files. ✓
- File structure → Tasks 1–16 create exactly the tree from the spec. ✓
- Variant signatures → Tasks 13 (warm: live dot, orange, 5 testimonials), 14 (serious: accent border, 2 verified), 15 (bairesdev: grayscale, no testimonials, no Agentic Designer). ✓
- Data flow → Task 4 loader, Task 16 orchestrator. ✓
- Schema additions → Tasks 1, 2, 3, 4. ✓
- Success criteria → Task 17 verifies all 8. ✓

**Placeholder scan:** no TBD, TODO, "similar to Task N", or bare "handle edge cases" lines. Every step has executable content.

**Type consistency:**
- `CvVariant = "warm" | "serious" | "bairesdev"` — used in Tasks 6, 7, 8, 9, 11, 13, 14, 15.
- `Lang = "en" | "es"` — imported from Phase 1 `skills-sheet-page-1.ts`.
- `ExperienceDensity = "full" | "condensed"` — used in Task 8 and consumed in Tasks 13, 14, 15.
- `renderWarmCv / renderSeriousCv` signatures `(data, lang, tokensCss) => string` consistent.
- `renderBairesdevCv` signature `(data) => string` (no lang, no tokens) — intentional, documented in test.
...
**Plan complete and saved to `docs/superpowers/plans/2026-05-07-phase2-cv-generators.md`. Two execution options:**
