# Playbook: Portfolio para Product Designer Senior / Staff / Principal

Complementa `cv-optimization-playbook.md` (CV/ATS). Este archivo cubre solo **portfolio**: que mostrar, como estructurarlo, y como evitar que te descarten en los primeros 6-8 segundos.

## Overview

El portfolio no prueba craft, prueba **judgment**: que puedes tomar una decision bajo incertidumbre, asumir la consecuencia, y mejorar la siguiente. En 2026, ejecucion pura es commodity (AI la abarato); lo que escasea es estrategia, criterio y la capacidad de dirigir AI con intencion. Nada de esto garantiza una oferta — el objetivo es maximizar probabilidad de entrevista, no asegurar el resultado.

**El portfolio abre la puerta, no cierra la decision.** Una porcion creciente de empresas de alto crecimiento ya salta el walkthrough pasivo de casos y abre Figma en vivo para verte resolver un problema ahi mismo. Implicacion practica: cada caso que muestres debe ser uno que puedas defender y extender en vivo bajo preguntas — si no podrias re-tomar esa decision frente a alguien que te interrumpe, no esta listo para el portfolio.

## Que escanean primero (6-8 segundos)

Filtro mental del recruiter/hiring manager, en este orden:
1. **Role fit** — que tipo de designer eres, en 3 segundos.
2. **Seniority signal** — se infiere scope, ownership, judgment (no el titulo que pongas).
3. **Relevance** — sirve para este rol especifico.

Si estos tres no quedan claros de inmediato, cierran la pestana. No hay segunda oportunidad para "se entiende mejor si sigues leyendo".

**Formula de headline que funciona:** "[Rol] · [anos de experiencia] · [dominio/industria]" (ej. "Product Designer · 6 anos · B2B SaaS & Fintech") supera por mucho a una frase de marca generica ("Diseno experiencias significativas"). El headline tiene que resolver rol + seniority + dominio en una linea, sin que el lector tenga que inferir nada.

**El primer fold es en si mismo evidencia de craft:** antes de leer un caso, el lector ya esta evaluando tu information architecture — agrupacion, espaciado, jerarquia visual de headline/credencial/proyectos. Un home desordenado se lee como "asi sera su UX work", independientemente del contenido. Refuerza el red flag #2 de mas abajo, pero aplica especificamente al home, no solo a la navegacion interna.

**Capa previa a humanos:** buena parte de recruiting ya corre un filtro AI-asistido antes de que un humano abra el portfolio (encuestas de practicas de hiring 2026 ubican esto en la mayoria de equipos de producto/diseno). La revision humana que sigue dura 2-3 minutos en total, con 10-15 segundos solo en la home. Implicacion practica: el texto de la home y el primer caso debe ser legible y parseable como texto plano (no solo como imagen/canvas), porque ese filtro automatizado lee contenido, no estetica.

**Segunda pasada (menos de 2 minutos), si pasa el filtro inicial:** abre el sitio -> lee la frase de presentacion -> entra al primer caso -> escanea su estructura -> salta directo a resultados. Implicaciones practicas: el primer caso debe ser el mas fuerte (no el cronologicamente mas reciente), y el resultado tiene que ser visible sin scroll profundo — si esta enterrado al final de un caso largo, no lo van a leer.

**El caso debe sostenerse sin abrir Figma.** Buena parte de recruiters y hiring managers no tiene cuenta de Figma a mano o no quiere salir del flujo de revision para abrirla. Si la unica forma de entender la decision es clickeando a un prototipo externo, la mayoria no lo va a hacer — el caso tiene que explicarse completo como pagina web, con el link a Figma como evidencia adicional, no como requisito para entender la historia.

## Como evaluan (hiring manager, segunda pasada)

- ¿El producto es real y trazable? ¿Sigue vivo, escalo? Un rediseno bonito que shippeo y no movio ninguna metrica es indistinguible de un proyecto concept — muestra craft, no judgment.
- ¿Que tomaste de decision, no que ejecutaste? Decision-making escala con seniority; ejecucion no.
- ¿Hay evidencia de proceso real: research, iteracion, testeo, feedback incorporado? Ausencia de esto es la red flag #1.
- ¿Puedes nombrar el tradeoff que rechazaste y por que? Si solo cuentas la version que "funciono", falta la mitad de la historia.

## El walkthrough en vivo (lo que pasa despues de que abren el portfolio)

- **El hiring manager llega preparado, no improvisa.** Un proceso de evaluacion publicado por un hiring manager describe ~15 minutos de revision previa a la llamada, donde marca 1-2 secciones especificas para profundizar y prepara preguntas de comportamiento sobre esas secciones. Implicacion practica: la pregunta dificil no es generica, cae sobre la seccion que se ve mas delgada o mas ambigua en una lectura rapida — si un caso del portfolio tiene un hueco, ese es el que van a interrogar.
- **Etapas tardias suelen usar panel cross-funcional** (eng/EM, PM, 2-3 designers) en una revision de ~60 minutos, no solo otros disenadores. Prepara una narracion que tenga sentido de negocio para alguien que no es designer, no solo terminologia de UX.
- **El fallo mas citado en esta etapa no es calidad de contenido, es falta de narracion preparada.** Candidatos que "simplemente scrollean" su propio sitio en vivo, sin un hilo verbal armado de antemano, se leen como incapaces de contar una historia coherente — incluso si el documento detras es fuerte. Practica en voz alta el recorrido de cada caso antes de la llamada; el portfolio escrito y el portfolio hablado son dos artefactos distintos que ambos necesitan preparacion.
- **"Craft deep dive": abren el archivo real, no solo miran el case study.** Una porcion creciente de hiring managers, despues de la narrativa, pide abrir el Figma/repo original y pregunta sobre decisiones granulares — tipografia, jerarquia de iconografia, espaciado, ritmo de un flujo — no solo la decision macro del caso. Implicacion practica: para cada caso que muestres, prepara la justificacion de 2-3 decisiones de detalle (no solo la decision estrategica), porque el caso escrito raramente las documenta y la pregunta llega sin aviso.

## Senior vs Staff vs Principal — tabla de decision

| Dimension | Senior | Staff | Principal |
|---|---|---|---|
| Pregunta que respondes | "¿Como lo construimos bien?" | "¿Deberiamos construir esto, y por que?" | "¿Hacia donde va el producto/negocio?" |
| Scope de impacto | Tu producto/feature | Equipo + equipos adyacentes (pillar) | Toda la org / multiples grupos |
| Rol en estrategia | Ejecuta la estrategia | Contribuye a la estrategia del grupo | Define y conduce la vision de producto |
| Multiplicador | De tu propio output | De tu equipo y equipos cercanos | De toda la organizacion |
| Relacion con liderazgo | Reporta hacia arriba | Pairea con leads de equipos | Pairea con liderazgo senior/exec |
| Como se ve en el portfolio | 1 caso fuerte con metricas claras de producto | Casos que muestran influencia cross-team sin autoridad formal | Casos que muestran como cambiaste la direccion de un negocio/org |

Nota de calibracion: el nivel es relativo al tamano de la empresa — un Principal en una startup de 100 personas puede equivaler a un Senior en una de 1500. No asumas que el titulo de tu rol anterior se traduce 1:1 al de la empresa objetivo; calibra el caso al scope real, no al nombre del cargo.

**Pregunta diagnostico para saber si un caso es Staff y no Senior:** ¿el caso resuelve "¿estan distintos equipos resolviendo el mismo problema de formas distintas?" o define un patron que otros equipos reutilizan? Si el caso solo describe una feature de tu propio equipo, es Senior aunque el titulo diga Staff. Dato de referencia de asignacion de tiempo: Senior gasta ~50-60% en ejecucion directa; Staff baja a ~25% porque el resto es influencia cross-team — si tu narrativa de caso es 90% "yo ejecute X", el portfolio se lee Senior independientemente del titulo que pongas.

**Red flag especifico de mismatch:** titulo Staff/Principal con casos que solo muestran trabajo de feature aislada, sin mencion de patrones compartidos, multiplicador de equipo o cambio de direccion. El nivel se demuestra en el contenido del caso, no se declara en el titulo de la pagina.

**"Senior Plateau":** varias companias en 2026 contratan mas Seniors pero promueven menos de Senior a Staff, porque la barra de Staff cambio de naturaleza, no solo de magnitud — dejo de ser "ejecutar mejor" y paso a ser "decidir si se construye". Un Senior con 6 anos de craft solido puede quedar estancado indefinidamente si su portfolio sigue probando solo la pregunta de Senior. Tactica concreta para destrabarlo: elegi una habilidad especifica de Staff que aun no tengas (fluidez de codigo, prototipado con AI, o diseno/evaluacion de criterios de calidad de modelo), shippea algo visible y publico con ella en ~8 semanas, y trata ese artefacto como el caso de portfolio para el proximo titulo — no esperes al ciclo de review para que el titulo alcance al trabajo.

**Estructura recomendada para casos Staff/Principal — dos secciones adicionales al framework de 6 bloques:**
- **"Strategic considerations"** — las tensiones de negocio que navegaste (ej. crecimiento vs deuda tecnica, velocidad vs compliance), no solo la solucion final. Es lo que distingue un caso de scope amplio de uno de feature aislada.
- **"Collaboration & impact"** — mapa breve de con quien trabajaste (eng leads, data, legal, otros design leads) y que resultado especifico salio de cada relacion. Nombra a las personas/equipos por su rol real; un logro de influencia sin nombrar a quien influenciaste se lee como afirmacion sin evidencia.

**Staff/Principal no significa "deje de ejecutar".** La expectativa 2026 en companias de alto crecimiento es "player-coach" / builder mindset: incluso en roles de liderazgo, se espera impacto directo y visible en el producto, no solo estrategia desde afuera. Un portfolio Staff/Principal que muestra cero trabajo hands-on (todo es "definí la direccion, el equipo construyo") se lee como desconectado del craft, no como mas senior. Mezcla al menos un caso con tu mano directa en la decision de diseno, ademas de los casos de influencia cross-team.

**Capa de credibilidad externa para Staff/Principal:** a ese nivel, el portfolio deja de ser la unica evidencia — charlas, escritura publica (newsletters, UX Collective, etc.) o liderazgo de comunidad funcionan como contrapeso a la abstraccion del trabajo de liderazgo, probando que "sigues siendo designer de corazon" ademas de gestionar. No es obligatorio, pero si lo tienes, enlazalo: compensa el hecho de que a este nivel hay menos pantallas que mostrar y mas decisiones de direccion dificiles de visualizar.

### Si el rol es de management/direccion (no IC)

El framework de 6 bloques de arriba es para IC. Un portfolio de Design Manager/Director que copia ese mismo esqueleto se lee desalineado con el rol — la evaluacion busca otra cosa.

- **Dos case studies, no uno:** un caso **estrategico** (vision, alineamiento con negocio, influencia en roadmap, metricas de impacto) y un caso **tactico** (calidad de craft/direccion creativa que tu equipo produjo bajo tu liderazgo). Completa con una seccion breve de "muestrario" para mostrar amplitud sin profundizar en cada uno.
- **Minimiza el "people management" en el documento.** Conteo de reportes directos, 1:1s, performance reviews — eso se cuenta en la entrevista en vivo, no en el portfolio. El documento es para criterio de producto y direccion, no para probar que sabes gestionar personas.
- **La entrega importa tanto como el contenido:** a este nivel el portfolio es "un acto de storytelling, no un demo de producto ni un reporte de status" — la narrativa en vivo (como lo presentas, no solo que pusiste en la pagina) es parte de la evaluacion. Muestra balance explicito entre ejes con tension real: operacion de equipo vs. innovacion de producto, estrategico vs. tactico, impacto de negocio vs. necesidad de usuario, breadth vs. depth, liderar desde el frente vs. desde atras. Nombrar estos ejes y donde te ubicas en cada uno es mas convincente que afirmar "soy un lider estrategico" sin evidencia del tradeoff.

## Estructura de case study (framework)

Seis bloques, en este orden. Cada uno debe poder leerse en 20-30 segundos si el lector tiene prisa.

1. **Contexto + tu rol** — negocio, producto, tu ownership especifico (no "we", se claro en que parte fuiste tu). Si el resultado fue de equipo, dilo explicito ("yo lidere X, el equipo de data construyo Y") — un logro de equipo presentado como individual se detecta en la entrevista de seguimiento, y se nota peor ahi que si lo hubieras aclarado desde el portfolio. Incluye el "trabajo antes del trabajo" si existio: como se definio el problema, quien estaba en la conversacion antes de que existiera un wireframe. Saltar directo a las pantallas oculta exactamente la parte que distingue judgment de ejecucion.
2. **El problema (con restricciones reales)** — datos de negocio, restricciones tecnicas/legales/de tiempo. Sin restricciones, no hay judgment que mostrar.
3. **Proceso** — research, hipotesis, iteracion, quien estuvo en la mesa (cross-functional). No describas el proceso, muestra una decision concreta que tomaste dentro de el.
4. **El tradeoff que rechazaste** — la alternativa descartada y por que. Esto es lo que separa judgment de craft.
5. **Resultado medible** — metrica de negocio o producto, no solo "mejoro la experiencia". Si el producto sigue vivo, dilo y di si escalo.
6. **Que harias diferente / que aprendiste** — honestidad calculada, no autocritica vacia.

### Ejemplo corto (formato, no caso real)
> **Contexto:** Fintech B2B, checkout de onboarding con 40% de drop-off en KYC.
> **Problema:** Legal exigia 6 campos no negociables; ingenieria tenia 2 sprints.
> **Decision:** Dividir el flujo en 2 pasos progresivos en vez de un formulario unico (alternativa descartada: autosave + un solo paso, rechazada por riesgo legal de datos incompletos).
> **Resultado:** Drop-off de 40% a 24% en 6 semanas; flujo sigue en produccion 18 meses despues.
> **Aprendizaje:** Subestime el tiempo de QA con legal; en el siguiente proyecto los incluí desde el research.

## Mostrando lo dificil de mostrar

- **Estrategia/systems thinking:** dibuja el sistema (no solo la pantalla) — interconexiones, feedback loops, puntos de apalancamiento. Una sola pantalla nunca prueba systems thinking; un mapa o diagrama si.
- **Metricas:** numeros de negocio (revenue, retention, conversion, costo evitado), no solo "satisfaction score". Si no tienes el numero exacto, usa el orden de magnitud y se honesto sobre la fuente. Para priorizacion de research/findings, un framework simple como severidad = criticidad de la tarea x impacto x frecuencia es mas convincente que una lista de issues sin orden — muestra como decidiste que atacar primero, no solo que encontraste.
- **Ambiguedad:** nombra explicitamente que informacion NO tenias y como decidiste sin ella. Eso es la prueba, no la pantalla final.
- **Liderazgo sin autoridad formal:** cuenta a quien convenciste, que resistencia hubo, como la resolviste. Liderazgo de IC se mide en influencia, no en headcount.
- **Tradeoffs:** un parrafo dedicado solo a "lo que NO hice y por que" en cada caso. Si no esta, falta.
- **Impacto de negocio:** conecta la metrica de producto con la de negocio (ej. "+8pp activacion -> +$420k ARR estimado"). El hiring manager de nivel senior+ piensa en P&L, no en pixeles.

## Red flags que descartan en el primer scan

1. Resultado sin "por que" — pantalla final sin problema, restricciones, ni decision.
2. Portfolio con mala UX propia (navegacion confusa) — descalifica de inmediato, es ironico pero real.
3. No se entiende que tipo de designer eres en los primeros 10 segundos.
4. Proyectos concept/fantasia (rediseno de Spotify/Airbnb sin usuarios reales ni restricciones de negocio) — muestra craft, no judgment.
5. Casos duplicados (3 checkouts rediseñados) — curado > comprensivo. 3 casos fuertes ganan a 7 mediocres.
6. Mismo patron visual aplicado a todo — sugiere eleccion por habito, no por intencion.
7. Cero evidencia de testing, research o iteracion con feedback real.
8. Metricas vagas ("mejoro la experiencia") sin numero, sin fuente, sin periodo de tiempo.
9. Mejora reportada sin baseline ("40% mejor" — ¿mejor que que?). Sin punto de comparacion explicito, el numero no significa nada y se lee como inflado.
10. "Shippeado a produccion" sin evidencia verificable (link, dashboard, captura de monitoreo, video corto). Es el claim mas comun y el primero que se chequea en entrevista — si no lo puedes mostrar, no lo afirmes asi.
11. Craft visual debil disfrazado de "yo soy mas de research/estrategia" — para la mayoria de roles de product design (no research puro), pulido visual flojo descalifica aunque el proceso sea solido. Es, en multiples reportes de hiring managers de 2026, la razon mas citada de rechazo en el primer filtro: la impresion visual de los primeros 3-5 segundos pesa mas que la calidad del proceso documentado debajo.
12. Cero mencion de como usas AI en tu proceso de diseno. En 2026 esto ya no se lee como neutral, se lee como senial de que no te has adaptado — encuestas recientes a managers de diseno ubican la fluidez con AI como competencia cada vez mas exigida (mayoria espera proficiencia en herramientas AI, y una proporcion similar espera capacidad de disenar productos AI), con premium salarial medible para quienes la demuestran con criterio (no como buzzword).
13. Exceso de texto de proceso o diagramas de double-diamond/journey-map genericos sin una decision concreta al lado. Estos diagramas estan sobreusados y ya no diferencian — si los usas, que sirvan para mostrar UNA decision especifica, no el proceso completo.
14. El template/design-system del portfolio le gana en atencion al trabajo mismo. Si el visitante recuerda el layout antes que el caso, el craft visual esta compitiendo contra tu propio contenido.
15. Estructura de caso indistinguible de un template generado por AI — el mismo esqueleto "contexto-problema-proceso-solucion-resultado" aplicado idéntico en los 5 casos, sin voz propia ni decision especifica que lo distinga de cualquier otro portfolio del mismo bootcamp. La cura no es el formato del case study, es la especificidad de la decision que cuentas dentro de el.
16. En casos AI, ausencia total de una seccion de limitaciones (que no resuelve el modelo, donde falla, que quedo fuera de scope). Reviewers de portfolios AI ya filtran por esto: su ausencia se lee como que no pensaste en los limites o los estas ocultando, no como que todo funciono perfecto.
17. Presentar output generado por AI (research sintetizado, conceptos, copy, incluso codigo de prototipo) como si fuera 100% trabajo manual propio, sin aclarar donde entro la AI. Es el reverso del red flag #12: tan danino omitir el uso de AI como ocultarlo cuando se uso. Se descubre en la entrevista de seguimiento ("camina por como llegaste a esto") y se lee peor que si lo hubieras declarado desde el caso. La frase segura: nombrar donde usaste AI y que decision de criterio tomaste sobre su output — eso es la prueba de fluidez, no la admision de un atajo.
18. Portfolio estatico, sin actualizar en mas de 6-12 meses, en un mercado de hiring que cambia rapido — un portfolio no es un archivo, es una herramienta de venta viva. Si el ultimo caso es de hace 2 anos o no menciona como trabajas hoy (incluyendo AI), se lee como senial de que no estas activo en el mercado o no has crecido desde entonces.
19. Estetica de portfolio generada por AI sin edicion propia — bento-grids identicos, gradientes oscuros genericos, copy de "soy un pensador estrategico" intercambiable entre candidatos. Esto es distinto del red flag #17 (ocultar uso de AI en el trabajo mostrado): aqui el problema es que el contenedor mismo del portfolio se ve generado, lo que erosiona confianza antes de leer una sola palabra del caso. Si usaste un builder con AI, edita lo suficiente para que no se vea identico a los otros 50 portfolios generados con el mismo prompt.
20. En el walkthrough en vivo, scrollear el portfolio sin un hilo narrado preparado de antemano. Un documento fuerte se lee debil si la version hablada es improvisada — son dos artefactos distintos, y solo uno se prepara escribiendo el otro.

## Patrones que funcionan (2026)

- **Video corto por caso (30-90s), no solo para AI:** la tendencia 2026 es hibrida — video al frente explicando la decision clave en menos de 90 segundos, mas un resumen de texto corto debajo, en vez de un case study largo de scroll infinito. Calza con el tiempo real de lectura humana (2-3 minutos totales) y obliga a comprimir la historia a lo esencial.
- **Formato "pensamiento en curso" abierto:** secciones que admiten que no sabes algo aun, documentando el proceso de pensar el problema en vivo — mas valorado que un caso pulido y cerrado, porque prueba judgment en tiempo real.
- **Evidencia de trabajo con AI como meta-skill:** un loom corto (3-5 min) mostrando como prompteas, editas, y tomas decisiones de criterio sobre output de AI. Para roles AI-adjacent esto pesa mas que un case study tradicional.
- **3-5 casos, no mas.** Cada uno con rol explicito, restricciones explicitas, y un tradeoff nombrado.
- **Thumbnails de proyecto con una linea de contexto, no solo el nombre.** "Rediseno de onboarding B2B — de 40% a 24% drop-off" en la tarjeta del proyecto filtra mejor que "Proyecto Checkout" — el lector decide si entrar al caso sin tener que abrirlo.
- **Pagina de "como pienso"** separada de los casos — un compendio corto de principios/heuristicas que usas, sirve como prueba adicional de judgment sin depender de un proyecto especifico.
- **Narrativa escrita corta (500-800 palabras):** alternativa o complemento al case study visual — traza una decision especifica desde el punto ambiguo de partida hasta el resultado, en prosa, sin diagramas pesados. Funciona mejor que un formato visual cargado cuando el lector tiene poco tiempo y quiere "verte pensar", no "ver pantallas".
- **Prototipo funcional, no solo mockup:** para roles staff/principal AI-adjacent, un prototipo con codigo real y datos reales (Framer, ProtoPie, o repo) que un ingeniero podria continuar pesa mas que una serie de pantallas en Figma — siempre que sea verificable (link vivo, repo, o video corto del flujo funcionando). Afirmar que algo es "shippable" sin poder mostrarlo es el red flag #10 de la lista anterior.
- **La herramienta del portfolio importa menos que la friccion que genera.** Hiring managers reportan que no les importa si es "una pagina de Notion o un sitio simple" — lo que descalifica es friccion de acceso (passwords, links rotos, navegacion custom rara) o presentacion gimmick que los obliga a aprender un formato nuevo para leer el caso. Framer (Figma a sitio, sin codigo) ya es un default aceptado para IC senior; Notion es valido pero se nota menos pulido que Webflow/Framer. Elige por friccion cero para el lector, no por la herramienta "correcta".
- **Prototipado con AI coding tools como senial de fluidez, no solo de velocidad:** mostrar que prototipas con Cursor, Claude Code, v0 o similar (no solo Figma) y que puedes acercarte al codigo de frontend real es una senial de seniority en 2026 — implica que puedes validar una idea sin esperar a ingenieria. El punto no es "uso AI para ir mas rapido", es "uso AI para tomar mejores decisiones de diseno con menos ciclos de espera".
- **Vocabulario de fluidez AI en roles staff+/principal AI-adjacent:** generative UI patterns, tokenizacion de design systems, comportamiento de modelos/LLM, patrones de diseno de sistemas agenticos. No hace falta dominarlo todo — mencionarlo con criterio real (no como buzzword suelto) es una senial de seniority que los JDs de 2026 ya piden explicitamente.
- **Caso de eval propio, sin necesitar un trabajo AI actual:** elegi una feature AI publica (ChatGPT, Claude, Notion AI), corre un eval de ~50 casos, y documenta que falla, que pasa, y las categorias del fallo. Es el ejercicio mas barato de producir y el que mas credibilidad da para roles AI-adjacent — diseño de evals es la habilidad menos preparada en entrevistas AI, y mostrarla en el portfolio es diferenciador inmediato.
- **Tres arquetipos validos de caso AI** (usa el que tengas, no fuerces los tres): (1) feature AI dentro de un producto existente (ej. smart replies); (2) producto AI-first desde cero (ej. herramienta generativa); (3) rediseno de un workflow completo alrededor de una feature AI (ej. automatizacion de edicion de video). Cualquiera de los tres es valido si muestra el "messy middle" — los quirks, limites y comportamiento impredecible del modelo — y no solo la pantalla final pulida.

## Tailoring por familia de rol

| Familia | Que enfatizar | Que evitar |
|---|---|---|
| B2B SaaS | Multi-stakeholder (sales, CS, eng), metricas de adopcion/retencion por segmento de cuenta, complejidad de permisos/roles | Casos B2C genericos sin contexto de venta enterprise |
| AI / AI-native products | Evals, golden datasets, criterios de "bueno" para comportamiento del modelo, manejo de incertidumbre/alucinacion, control del usuario sobre output de AI | Hablar solo de "UI de chat" sin mencionar evaluacion o criterios de calidad del modelo |
| Fintech | Confianza, compliance (KYC/AML), manejo de riesgo regulatorio, claridad bajo presion legal | Casos que ignoren restricciones legales o de seguridad |
| Enterprise / regulado (salud, legal, gobierno) | WCAG/accesibilidad, criterios de auditoria, ciclos de aprobacion largos, diseño de sistemas (no pantallas sueltas) | Estetica como argumento principal; falta de evidencia de compliance |
| Government / sector publico | Accesibilidad estricta, proceso de procurement, multiples stakeholders no tecnicos, lenguaje plano | Jerga de startup, metricas de growth que no aplican al contexto |

Calibra cada caso eligiendo 1-2 filas relevantes al rol objetivo; no fuerces todas las dimensiones en un solo caso.

**Profundidad sobre amplitud:** en procesos con cientos de aplicantes, la minoria que calza con los requisitos especificos del rol gana por profundidad en ese dominio, no por cobertura general — y esa minoria no es la mas experimentada, es la mas especifica. Un portfolio generico que toca todas las familias de rol pesa menos que uno afilado a una sola, con vocabulario y metricas nativas de esa industria.

## Checklist pre-publicacion

- [ ] En los primeros 10 segundos, ¿se entiende que tipo de designer soy y mi seniority?
- [ ] Cada caso tiene: contexto + rol propio, problema con restricciones reales, proceso, tradeoff rechazado, resultado medible, aprendizaje.
- [ ] Hay al menos un numero de negocio (no solo de producto) por caso.
- [ ] Elimine duplicados — quedan 3-5 casos curados, no todos los proyectos que hice.
- [ ] Cada caso esta calibrado al nivel objetivo (senior/staff/principal) segun la tabla de scope.
- [ ] Si aplico a rol AI-adjacent, hay evidencia de manejo de evals/criterios de calidad de modelo, o un loom de proceso con AI.
- [ ] La navegacion del portfolio mismo es clara — lo probé como si fuera un usuario nuevo.
- [ ] Ningun caso es "concept"/fantasia sin restricciones reales de negocio.
- [ ] El portfolio esta tailored al role family del puesto especifico (tabla de arriba), no generico.
- [ ] El nivel que proyecto en el portfolio coincide con el nivel del JD al que aplico (no aplico mas senior ni mas junior de lo que el caso realmente demuestra).
- [ ] Ningun caso es solo texto/diagrama de proceso sin una decision concreta — si hay double-diamond o journey map, esta ahi para mostrar UNA decision, no el proceso completo.
- [ ] Si aplico a rol de management/direccion, el portfolio sigue la estructura estrategico+tactico (no el framework de 6 bloques de IC), y el people-management se quedo para la entrevista, no para el documento.
- [ ] Practique en voz alta la narracion de cada caso — no voy a "scrollear" el sitio en vivo sin un hilo armado de antemano.

## Changelog

- **2026-06-26** — Creacion inicial del playbook. Investigacion sobre que buscan recruiters/hiring managers en 2026 (judgment > craft, evals para AI design, formato "pensamiento abierto"), diferencias senior/staff/principal por scope, framework de case study en 6 bloques, red flags, y tabla de tailoring por familia de rol (B2B SaaS, AI, fintech, enterprise/regulado, gobierno).
- **2026-06-26** — Pasada de research adicional. Agregado: modelo de doble pasada de revision (6-8s filtro + <2min lectura real, con implicancia de que el primer caso debe ser el mas fuerte); atribucion explicita de contribucion individual vs equipo en el bloque de contexto; 4 red flags nuevos (mejora sin baseline, "shippeado" sin evidencia verificable, craft visual debil, ausencia de mencion de AI en el proceso); patrones nuevos de prototipo funcional verificable y vocabulario de fluidez AI (generative UI, tokenizacion, sistemas agenticos) para roles staff+/principal AI-adjacent.
- **2026-06-26 (pasada 3, UTC)** — Investigacion sobre filtro AI-asistido de recruiting (capa previa al humano, home debe ser legible como texto plano); pregunta diagnostico concreta para distinguir Staff de Senior (patrones cross-team vs feature aislada) con dato de asignacion de tiempo (Senior ~50-60% ejecucion vs Staff ~25%); 2 red flags nuevos (exceso de diagramas double-diamond/journey-map genericos sin decision concreta, template del portfolio compitiendo con el trabajo); ejercicio concreto de eval propio sobre feature AI publica (ChatGPT/Claude/Notion AI, ~50 casos) como proyecto de portfolio AI-adjacent sin requerir trabajo AI actual; 3 arquetipos validos de caso AI (feature embebida, producto AI-first, rediseno de workflow) con enfasis en mostrar el "messy middle" del modelo, no solo la pantalla final.
- **2026-06-26 (pasada 4, UTC)** — Nota en Overview sobre el shift a entrevistas con resolucion en vivo en Figma (el portfolio abre la puerta, no cierra la decision); 2 red flags nuevos (estructura de caso indistinguible de un template generado por AI, ausencia de seccion de limitaciones en casos AI); patron nuevo de video corto por caso (30-90s, hibrido con texto) generalizado mas alla del loom especifico de proceso AI; nota de profundidad-sobre-amplitud en Tailoring (especialistas con vocabulario nativo de la industria ganan sobre generalistas en procesos con muchos aplicantes); refuerzo del red flag de ausencia de AI con contexto de encuestas a managers de diseno.
- **2026-06-26 (pasada 5, UTC)** — Nuevo punto practico: el caso debe sostenerse sin requerir abrir Figma (muchos reclutadores no tienen cuenta o no quieren salir del flujo de revision); red flag nuevo #17 — presentar output generado por AI como 100% trabajo manual sin aclarar donde entro la AI (reverso del red flag de omitir AI por completo, igual de danino, se descubre en la entrevista de seguimiento); refuerzo de "profundidad sobre amplitud" (la minoria que calza con el rol especifico no es la mas experimentada, es la mas especifica); ajuste de tono en menciones de encuestas de hiring 2026 para no citar cifras puntuales sin fuente verificable directa.
- **2026-06-26 (pasada 6, UTC)** — Nota nueva en Senior/Staff/Principal: expectativa 2026 de "player-coach"/builder mindset — incluso en roles de liderazgo se espera impacto hands-on visible, un portfolio Staff/Principal sin ningun caso de mano directa en la decision de diseno se lee desconectado del craft; sharpened red flag #11 con framing de multiples reportes de hiring managers ("weak visual craft" como razon mas citada de rechazo en el primer filtro, independiente de la calidad del proceso); patron nuevo de prototipado con AI coding tools (Cursor, Claude Code, v0) como senial de fluidez que acelera la validacion de decisiones de diseno, distinto del prototipo funcional generico ya documentado.
- **2026-06-26 (pasada 7, UTC)** — Formula concreta de headline ("[Rol] · [anos] · [dominio]") con ejemplo, para cerrar el gap de "se entiende el role fit en 3 segundos"; nota nueva de que el primer fold es en si mismo evidencia de IA skills (espaciado/jerarquia del home se lee como proxy de calidad del UX work); dos secciones estructurales nuevas para casos Staff/Principal ("Strategic considerations" para tensiones de negocio, "Collaboration & impact" para mapa de stakeholders con resultado especifico por relacion, nombrando personas/equipos en vez de afirmar influencia sin evidencia); patron nuevo de thumbnails de proyecto con una linea de contexto medible en vez de solo el nombre del proyecto.
- **2026-06-26 (pasada 8, UTC)** — Concepto de "trabajo antes del trabajo" (alineacion y definicion del problema previas al wireframe) agregado al bloque 1 del framework de case study, como evidencia de involucramiento estrategico que se pierde si el caso salta directo a pantallas; framework concreto de priorizacion para research/findings (severidad = criticidad x impacto x frecuencia) en la seccion de metricas; red flag nuevo #18 — portfolio estatico sin actualizar en 6-12+ meses en un mercado de hiring que cambia rapido (un portfolio no es un archivo). Fuentes revisadas: Verified Insider (Tom Scott, "8 Red Flags" / "6 Green Flags"), UX Planet ("11 Hiring Red Flags 2026").
- **2026-06-26 (pasada 9, UTC)** — Seccion nueva para roles de management/direccion: framework distinto al de IC (caso estrategico + caso tactico + muestrario de breadth, people-management reservado para la entrevista en vivo, balance explicito entre ejes con tension como tactico vs. estrategico y breadth vs. depth); seccion nueva sobre el walkthrough en vivo (el hiring manager pre-marca 1-2 secciones para profundizar en una revision previa de ~15min, paneles cross-funcionales en etapas tardias, narracion preparada vs. "scrollear en vivo" como fallo de delivery distinto de la calidad del documento); nota de capa de credibilidad externa (charlas, escritura publica) para Staff/Principal como contrapeso a la abstraccion del trabajo de liderazgo; red flags nuevos #19 (estetica de portfolio generada por AI sin edicion, bento-grids/gradientes genericos, distinto del red flag de ocultar uso de AI en el trabajo mismo) y #20 (scrollear sin narracion preparada en el walkthrough); refinamiento del patron de plataforma — la friccion de acceso (passwords, links rotos, gimmicks) importa mas que la herramienta elegida (Framer ya es default aceptado, Notion valido pero menos pulido). Fuentes: Hannah Hearth ("The sprawl of design manager portfolios"), Helena Seo/DoorDash ("Three tips on design leaders' portfolio presentations"), Tom Scott (Verified Insider), Medium/Design Bootcamp ("My portfolio evaluation process").
- **2026-06-26 (pasada 10, UTC)** — Punto nuevo en el walkthrough en vivo: "craft deep dive" — el hiring manager abre el archivo real (Figma/repo) y pregunta por decisiones granulares (tipografia, iconografia, espaciado, ritmo) ademas de la decision macro del caso, asi que hay que preparar la justificacion de 2-3 detalles por caso, no solo la narrativa estrategica; concepto nuevo de "Senior Plateau" en la seccion Senior/Staff/Principal — companias contratan mas Seniors pero promueven menos a Staff porque la barra de Staff cambio de naturaleza ("ejecutar mejor" -> "decidir si se construye"), con tactica concreta de destrabe (elegir una habilidad especifica de Staff, shippearla publicamente en ~8 semanas, usarla como caso de portfolio para el proximo titulo en vez de esperar el ciclo de review). Fuentes: Nurkhon Nurxmedov ("Senior designers stopped becoming Staff in 2026"), The Fountain Institute ("What Actually Gets Senior Product Designers Hired in 2026"), Tom Scott / Verified Insider (FAQ).
