...
dr-cv/
â”œâ”€â”€ perfil/data/
â”‚   â”œâ”€â”€ positioning.yaml                 # MODIFY: add thesisBairesdev.en
...
â”œâ”€â”€ generadores/
â”‚   â”œâ”€â”€ lib/
â”‚   â”‚   â”œâ”€â”€ types.ts                     # MODIFY: extend Positioning + Skills
â”‚   â”‚   â””â”€â”€ load-data.ts                 # MODIFY: parse new fields
â”‚   â””â”€â”€ templates/cv/
â”‚       â”œâ”€â”€ components/
â”‚       â”‚   â”œâ”€â”€ skills-sidebar.ts        # MODIFY: reorder groups for bairesdev variant
...
**Files:**
- Modify: `generadores/lib/load-data.ts:125-148` (validatePositioning)
- Modify: `generadores/lib/load-data.ts:70-103` (validateSkills)
...
```ts
import { validateSkills } from "../generadores/lib/load-data.js";

describe("validatePositioning â€” thesisBairesdev", () => {
  it("accepts optional thesisBairesdev.en", () => {
    // this will be re-loaded via loadAllData once perfil/data/positioning.yaml ships it;
    ...
    const { loadAllData } = require("../generadores/lib/load-data.js");
...
In `generadores/lib/load-data.ts`, replace the `validateSkills` function body. Find the `return { byLayer: ..., byOutcome: ... }` block (around line 100â€“103) and change it to:
...
```bash
git add generadores/lib/load-data.ts tests/load-data.test.ts
...
**Files:**
- Modify: `perfil/data/skills.yaml` (append new root key)

- [ ] **Step 1: Append inventory to skills.yaml**

Open `perfil/data/skills.yaml`. Append a new root key at the bottom of the file (after the `byOutcome` block ends at line ~81):
...
Run: `npx vitest run tests/load-data.test.ts`
Expected: PASS â€” `cv.skills.inventory.length` â‰¥ 20, `UX/UI` â†’ `10+`.

- [ ] **Step 4: Commit**

```bash
git add perfil/data/skills.yaml tests/load-data.test.ts
...
**Files:**
- Modify: `perfil/data/positioning.yaml` (append new root key)

- [ ] **Step 1: Append the new thesis**

Open `perfil/data/positioning.yaml`. After the closing `proofNumbers` list (line ~22), append at root level:
...
```bash
git add perfil/data/positioning.yaml tests/load-data.test.ts
...
**Files:**
- Create: `generadores/templates/cv/components/skills-inventory.ts`
...
Note: `SkillInventoryItem` must be exported from `generadores/lib/types.ts` (done in Task 1).
...
```bash
git add generadores/templates/cv/components/skills-inventory.ts tests/cv-components.test.ts
...
**Files:**
- Modify: `generadores/templates/cv/components/skills-sidebar.ts`
...
Open `generadores/templates/cv/components/skills-sidebar.ts`. Replace the function body:
...
```bash
git add generadores/templates/cv/components/skills-sidebar.ts tests/cv-components.test.ts
...
**Files:**
- Modify: `generadores/templates/cv/bairesdev.ts`
...
Open `generadores/templates/cv/bairesdev.ts`. Add an import near the other component imports at the top:
...
Still in `generadores/templates/cv/bairesdev.ts`, locate the second `<article class="cv-page">` block and its `<aside>` element (around line 477):
...
- Page 2 left aside shows Education first, then Skills Inventory second, with â‰¥ 20 skill rows.
...
```bash
git add generadores/templates/cv/bairesdev.ts tests/cv-bairesdev.test.ts
...
| Webpack                | 2+    | VÃ­a Next.js / Vite toolchain                  |
...
| C/C++               | Leave blank    | Sin prÃ¡ctica reciente                   |
...
- Skills Inventory visible on page 2 left aside, with â‰¥ 20 rows fitting without overflow.

- [ ] **Step 6: Final commit (if layout tweaks were needed in Step 4 or 5)**

If any final CSS tuning was required in `generadores/templates/cv/bairesdev.ts` to keep 2 pages:

```bash
git add generadores/templates/cv/bairesdev.ts
...
1. âœ… `npm run build:cvs` regenerates `cv-bairesdev-en.{html,pdf}` with the new thesis and skills inventory without breaking the other two CV variants. â€” covered by Task 7 Step 8 + Task 9 Step 3.
2. âœ… The two-page layout stays intact â€” no orphan page 3. â€” covered by Task 7 Step 9 + Task 9 Step 5.
3. âœ… All three test files pass. â€” covered by Task 9 Step 1.
4. âœ… The Markdown checklist `docs/deliverables/bairesdev-portal-skills.md` exists, is committed, and is readable as a standalone document. â€” covered by Task 8.
5. âœ… A recruiter reading both the CV and the portal sees a consistent story. â€” qualitative; achieved by matching inventory labels to portal taxonomy (Task 3) and aligning thesis (Task 4).
