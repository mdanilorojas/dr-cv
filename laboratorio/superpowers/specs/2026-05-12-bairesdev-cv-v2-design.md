# BairesDev CV v2 + Portal Skills Inventory â€” Design Spec
...
**Predecessor:** `2026-05-07-phase2-cv-generadores-design.md` (CV generadores, shipped)
...
1. **The BairesDev portal skills inventory** â€” a taxonomy-bound list (skill + years) that their matching system uses to route positions. Danilo has already declared skills there, with several inconsistencies (e.g. HTML 4+, JavaScript 1+, Agile 4+ â€” lower than reality) and blanks (several "Select years" rows).
2. **The BairesDev CV variant** â€” `dist/cvs/cv-bairesdev-en.{html,pdf}`, generated by `generadores/templates/cv/bairesdev.ts`, currently positions Danilo as "Agentic Designer Â· Product Engineer" with thesis *"I ship real products â€” and I ship the tools agents use to help me ship them."*

The tension: the portal reads like a conservative mid-senior frontend + UX/UI generalist. The CV reads like a founder-operator. A recruiter comparing the two detects dissonance. Worse â€” the portal under-represents Danilo's actual years (HTML/JS 10+, Figma 8+, Agile 10+) and lacks AI/agentic skills entirely, so BairesDev's matching engine will never route him to AI-forward positions.
...
### Decision 1 â€” Positioning: agentic-forward, frontend as reinforcement

Danilo's explicit direction: *"lo mÃ¡s importante para mÃ­ es dar esa perspectiva AI"*. Agentic framing leads; UX/UI + frontend appear lower as corroboration of the 15-year shipping track record.

**New thesis (EN):** *"Agentic Designer. 15 years delivering product â€” today I ship SaaS, design systems, and frontend with agents as force multiplier."*

### Decision 2 â€” Sidebar: agentic group first, inventory on page 2

The sidebar on page 1 keeps its four groups but reorders them so `Agents & AI-Assisted Delivery` is first, then `Design & Systems`, `Engineering`, `Strategy`. The existing `Strategy / Design / Engineering / Agents` groups in `perfil/data/skills.yaml` get relabeled and reordered; no items removed.

A new `Skills Inventory` block appears on page 2, above `Past Experience`, formatted as a compact 2-column `skill Â· years` grid that mirrors the BairesDev portal taxonomy. This is what the recruiter scans for matching.

### Decision 3 â€” Skills inventory baseline: "Realista amplio"
...
## Deliverable A â€” Regenerated BairesDev CV

### File changes

| File | Change |
|---|---|
| `perfil/data/positioning.yaml` | Add `thesisBairesdev.en` field with the new agentic-forward thesis. Keep existing `thesis` for warm/serious variants. |
| `perfil/data/skills.yaml` | Add `inventory` root key: an ordered list of `{ skill, years, notes? }` matching the BairesDev portal taxonomy. |
| `generadores/lib/types.ts` | Extend `PositioningData` with optional `thesisBairesdev`. Extend `SkillsData` with `inventory`. |
| `generadores/lib/load-data.ts` | Parse the two new fields. |
| `generadores/templates/cv/components/skills-sidebar.ts` | When `variant === "bairesdev"`, render groups in order `Agents â†’ Design â†’ Engineering â†’ Strategy`. Relabel `Agents` heading to `Agents & AI-Assisted Delivery`. No changes for other variants. |
| `generadores/templates/cv/bairesdev.ts` | Use `data.positioning.thesisBairesdev.en ?? data.positioning.thesis.en` in `renderSummaryBairesdev`. Add a new `renderSkillsInventory(inventory)` section on page 2, inserted between `renderEducationBlock` (left column) and `Past Experience` (right column). |
...
The `Skills Inventory` block uses a two-column `grid-template-columns: 1fr auto` where skill name is left-aligned and years right-aligned in mono font â€” same visual system as the `cv-edu__item` rows, so it reads as a continuation of the aside.

### Component contract

```ts
// new function in generadores/templates/cv/components/skills-inventory.ts
...
```yaml
# perfil/data/skills.yaml (new section, appended)
inventory:
...
```yaml
# perfil/data/positioning.yaml (new field)
thesisBairesdev:
...
## Deliverable B â€” Portal skills update checklist
...
Skills currently in Danilo's portal with years that under-represent his actual experience. Table columns: *Skill Â· Current Â· New Â· Reason*.

| Skill | Portal | Update to | Reason |
|---|---|---|---|
| Sales/Business Development | 10+ | **5+** | More honest for formal business-dev work |
| Agile Methodologies | 4+ | **10+** | Xentinels 2016â€“present + BAH + EnRegla |
...
| Skill | Years | Reason |
|---|---|---|
| Artificial Intelligence | 2+ | Diferenciador principal; 2 aÃ±os de prÃ¡ctica documentada |
...
| Anthropic Claude API | 1+ | Ya identificado por Danilo como bÃºsqueda alterna |
...
| Vercel | 1+ | Deployment para dr-cv y laboratorio EnRegla |
...
- `tests/cv-bairesdev.test.ts` â€” three new assertions:
...
- `tests/data.test.ts` â€” one new assertion: `skills.yaml` exports `inventory` as a non-empty array.
...
2. The two-page layout stays intact â€” no orphan page 3.
...
None â€” all decisions locked in brainstorming.
