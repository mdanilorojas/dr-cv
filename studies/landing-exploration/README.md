# Landing exploration — archived studies

Historical iterations of the landing page, kept for reference.

**Not part of the production build.** These generators are excluded from
`tsconfig.json` and `vitest.config.ts`, so their imports may be stale.
The production landing lives in `generators/v11-landing.ts` →
`dist/landing-v11/`.

## What's here

- `generators/landing.ts` — Phase 3 SPA with 5 tabs (EN + ES). The first
  shipped version.
- `generators/huly-landing.ts` — Huly-style exploration, single-page with
  heavier motion.
- `generators/templates/landing/` and `templates/huly-landing/` — their
  template code and component breakdowns.
- `tests/` — the Vitest suites that covered these when they were live.

## Reviving one of these

If you want to resurrect a study:

1. Move it back under `generators/` (and its tests to `tests/`).
2. Fix relative imports (they currently point to `../lib/` paths that no
   longer match once moved deeper).
3. Re-add its `build:…` script to `package.json`.
...
These studies contain decisions that informed v11 — component patterns,
