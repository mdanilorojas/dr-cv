...
**Goal:** Ship `npm run build:landing` producing `dist/landing/index.html` (EN) and `dist/landing/es/index.html` (ES). Single-file tab-SPA per language, reusing `/data/*` from Phases 1–2, Huly-aesthetic dark-dominant, 5 tabs, mobile-first, Lighthouse ≥ 90/95/95/95.

**Architecture:** Pure components `(data, lang) => string` in `generators/templates/landing/components/`. One orchestrator `generators/landing.ts`. Inline CSS via `landing-styles.ts`, inline JS via `landing-script.ts` (tab switching, hash sync, copy-to-clipboard, reduced-motion handling). Forked token set `design-system/tokens-landing.css` (dark-dominant) separate from existing `tokens.css` (light paper).

**Tech Stack:** Same as Phases 1–2 — Node 20+, TypeScript strict, js-yaml, vitest, tsx. NEW: puppeteer used for Lighthouse smoke test (lazy import to keep test suite fast); no framework, no bundler, no router.
...
1. `npm run build:landing` emits both HTML files from `/data/*` only.
...
5. Lighthouse ≥ 90 performance, ≥ 95 accessibility on `dist/landing/index.html`.
...
```
dr-cv/
├── package.json                                       # MODIFY: add build:landing, build:all includes it
├── data/
│   └── landing.yaml                                   # NEW: tabs config + CTA + SEO
├── design-system/
│   ├── tokens.css                                     # existing (light) — untouched
│   └── tokens-landing.css                             # NEW (dark-dominant)
├── assets/
│   └── fonts/                                         # NEW (self-hosted woff2, subset Latin-1)
│       ├── Inter-Regular.woff2
│       ├── Inter-Medium.woff2
...
├── generators/
│   ├── lib/
│   │   ├── types.ts                                   # MODIFY: add Landing, LandingTab, LandingData types
│   │   ├── load-data.ts                               # MODIFY: add loadLanding + loadLandingData (composes CvData + landing)
│   │   └── render-pdf.ts                              # existing — untouched
│   ├── templates/
│   │   └── landing/                                   # NEW
│   │       ├── components/
│   │       │   ├── tab-nav.ts                         # NEW
│   │       │   ├── hero-block.ts                      # NEW
│   │       │   ├── proof-numbers.ts                   # NEW
│   │       │   ├── case-expander.ts                   # NEW
│   │       │   ├── method-diagram.ts                  # NEW
│   │       │   ├── timeline.ts                        # NEW
│   │       │   ├── testimonial-featured.ts            # NEW
│   │       │   ├── section-visual.ts                  # NEW
│   │       │   ├── contact-block.ts                   # NEW
│   │       │   └── me-agent-dock.ts                   # NEW (empty placeholder for Phase 4+)
│   │       ├── landing-styles.ts                      # NEW (inline <style> blob)
│   │       ├── landing-script.ts                      # NEW (inline <script> blob)
│   │       └── index.ts                               # NEW (full-page template composer)
│   └── landing.ts                                     # NEW orchestrator
├── tests/
│   ├── landing-components.test.ts                     # NEW
│   ├── landing-integration.test.ts                    # NEW
│   └── landing-smoke.test.ts                          # NEW (puppeteer + lighthouse, opt-in via env)
└── dist/landing/                                      # NEW (gitignored via existing dist/ rule)
    ├── index.html
    ├── es/
    │   └── index.html
    └── assets/
        └── fonts/                                     # copied from /assets/fonts at build time
```
...
**Context:** Landing needs new `data/landing.yaml` that defines tab order, per-section visual hint, CTA text, SEO titles/descriptions per language. Everything else is reused from Phase 2's `CvData`.

- [ ] Create `data/landing.yaml` with schema from spec (tabs array of 5, cta en/es, seo en/es).
- [ ] Add TypeScript types in `generators/lib/types.ts`:
  ```ts
...
**Context:** `tokens.css` is optimized for light paper (CVs, skills sheet). Landing needs a distinct dark-dominant Huly palette. Fork cleanly.
...
- [ ] Implement `generators/templates/landing/components/tab-nav.ts`.
...
- [ ] Implement `hero-block.ts` and `proof-numbers.ts`. Reuse structure from existing CV components where sensible (identity-block has splitName helper — reuse directly).
...
**Context:** Reads from existing `data/cases/*.md` (already loaded by Phase 2's `loadCases`). Expansion is inline (no modal). Clicked-state is managed in runtime JS; component emits the full collapsed + expanded content with `aria-expanded="false"` initial state.
...
- [ ] `method-diagram.ts`: pure SVG, 4 nodes (Claude Code → MCP → Subagents → Skills-lock) with accent-colored arrows. ES version swaps labels via `lang` argument.
...
- [ ] `me-agent-dock.ts`: empty `<aside id="me-agent-dock" hidden aria-hidden="true"></aside>` — Phase 4 hook only.
- [ ] `section-visual.ts`: switch on `tab.visual` — each visual returns a CSS-only decorative `<div>` marked `aria-hidden="true"` (particles = radial gradient dots; grid = CSS grid lines; flow = linear-gradient stripe; timeline = dashed line; signature = text ornament). Keep tiny; heavy canvas deferred to Phase 4 if ever.
...
**Context:** Single blob of CSS. Imports `tokens-landing.css` by reading it at generator time (like `cv.ts` does with `tokens.css`) and concatenating.

- [ ] Implement `landing-styles.ts` exporting `LANDING_STYLES: string` covering:
  - Reset, body bg `var(--bg-ink)`, color `var(--ink-inverse)`.
  - `.lp-page` layout — mobile-first, max-width 1200px, padding clamp-based.
  ...
  - Media query: mobile ≤ 640px stacks everything vertical.
...
- [ ] Implement `generators/templates/landing/index.ts`:
...
- [ ] Implement `generators/landing.ts`:
...
- [ ] Add script `"build:landing": "tsx generators/landing.ts"` to `package.json`.
...
- [ ] Create `tests/landing-integration.test.ts`:
  ...
  - HTML length ≤ 400_000 bytes for each language.
...
- [ ] Create `tests/landing-smoke.test.ts`:
  ...
  - Runs Lighthouse (lazy import `lighthouse` package — add as devDep only if needed).
  - Asserts perf ≥ 90, a11y ≥ 95.
  - If lighthouse adds too much install weight, ship without it — manual Chrome DevTools check is acceptable, just document in plan completion.
...
- [ ] `npm test` → 97 existing + ~25 new = ~122 tests, all green.
- [ ] `npm run build:all` → skills-sheet + 5 CVs + 2 landing HTMLs produced.
- [ ] Manual Chrome Lighthouse on `dist/landing/index.html` — perf ≥ 90, a11y ≥ 95 captured in commit message.
