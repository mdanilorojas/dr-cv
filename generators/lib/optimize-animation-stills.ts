 ...
 *
 * For each .png in `assets/animations/<case>/`:
 ...
 *
 * Run with: npx tsx generators/lib/optimize-animation-stills.ts
 */
...
async function optimizeDir(dir: string): Promise<void> {
  for (const entry of readdirSync(dir)) {
    ...
    console.log(
      `[anim] ${path.relative(ROOT, full)} → ${path.relative(ROOT, out)}  ${(before / 1024).toFixed(0)}KB → ${(info.size / 1024).toFixed(0)}KB  (${meta.width}×${meta.height})`,
    );
  }
}
