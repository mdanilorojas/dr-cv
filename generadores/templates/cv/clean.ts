...
const ROWS: Row[] = [
  { years: "2022—NOW", co: "Booz Allen Hamilton", badge: "Army · DoD · VA · FAA",
    ...
    desc: {
      en: "Lead product designer for the federal Trusted Environments platform and Developer Portal, architecting two design systems in Figma (Variables, Auto Layout, Dev Mode) on Shadcn, Radix and the BAH Brand Book, compliant with WCAG 2.2 and Section 508 — passing 95%+ of automated accessibility checks. Pioneered agentic UX research: built LLM-based persona emulators to run moderated usability testing (5 users per feature), with 92% rating the resulting improvements better. Designed a custom MCP for developers; design system scaled from 0 to 26 internal teams, including Vellox Reverser (cybersecurity).",
      es: "Diseñador de producto principal para la plataforma federal Trusted Environments y el Developer Portal, construyendo dos design systems en Figma (Variables, Auto Layout, Dev Mode) sobre Shadcn, Radix y el BAH Brand Book, conformes con WCAG 2.2 y Sección 508 — superando el 95% de checks automatizados de accesibilidad. Fui pionero en investigación de UX agéntica: construí emuladores de personas basados en LLM para correr pruebas de usabilidad moderadas (5 usuarios por feature), con 92% de aprobación sobre las mejoras resultantes. Diseñé un MCP propio para developers; el design system escaló de 0 a 26 equipos internos en Booz Allen Hamilton, incluyendo el producto de ciberseguridad Vellox Reverser."
    } },
  { years: "2026—NOW", co: "Compliance SaaS · LATAM", badge: "280+ paying users",
    ...
    desc: {
      en: "Led discovery-to-launch design for a compliance SaaS for LATAM SMBs, shipping a validated MVP in 40 days. Instrumented end-to-end product analytics in Mixpanel, drove onboarding redesign that raised new-user activation from 2.6% to 9.4% in under 3 weeks, and ran an A/B test on sign-up (email vs. Google) — Google won, lifting onboarding completion 74%. Designed a 31-component Figma design system across 16 screens, implemented in React/TypeScript, growing the product to ~280 paying customers.",
      es: "Lideré el diseño de discovery a lanzamiento de un SaaS de cumplimiento para PYMES LATAM, validando el MVP en 40 días. Instrumenté analytics de producto end-to-end en Mixpanel, conduje el rediseño de onboarding que subió la activación de nuevos usuarios de 2,6% a 9,4% en menos de 3 semanas, y corrí un A/B test de registro (email vs. Google) — ganó Google, subiendo la finalización del onboarding en 74%. Diseñé un design system de 31 componentes en Figma en 16 pantallas, implementado en React/TypeScript, creciendo el producto a ~280 clientes de pago."
    } },
