...
const ROWS: Row[] = [
  { years: "2022—NOW", co: "Booz Allen Hamilton", badge: "Army · DoD · VA · FAA",
    ...
    desc: {
      en: "Lead product designer for the Trusted Environments federal platform and Developer Portal, architecting two design systems (Shadcn, Radix, BAH Brand Book) compliant with WCAG 2.2 and Section 508 — 95%+ of automated accessibility checks passed. Pioneered agentic UX research: conducted moderated usability tests (5 users per feature) with LLM-based persona emulators; 92% of participants rated resulting improvements better. Cut design-to-dev cycle from ~6 weeks to under 1 week by delivering near-production React components (Figma Variables · Dev Mode · Auto Layout → code). Design system scaled from 0 to 26 internal teams; designed a custom MCP for developers.",
      es: "Diseñador de producto principal para la plataforma federal Trusted Environments y el Developer Portal, construyendo dos design systems (Shadcn, Radix, BAH Brand Book) conformes con WCAG 2.2 y Sección 508 — 95%+ de checks automatizados aprobados. Fui pionero en investigación de UX agéntica: conduje pruebas de usabilidad moderadas (5 usuarios por feature) con emuladores de personas basados en LLM; 92% calificó las mejoras resultantes como mejores. Reduje el ciclo de diseño a desarrollo de ~6 semanas a menos de 1 semana entregando componentes React casi listos (Figma Variables · Dev Mode · Auto Layout → código). El design system escaló de 0 a 26 equipos internos; diseñé un MCP propio para developers."
    } },
  { years: "2026—NOW", co: "Compliance SaaS · LATAM", badge: "280+ paying users",
    ...
    desc: {
      en: "Lead product designer for a compliance SaaS for LATAM SMBs, owning discovery-to-launch design and shipping a validated MVP in 40 days. Instrumented Mixpanel end-to-end and defined the activation metric; onboarding redesign lifted new-user activation from 2.6% to 9.4% in under 3 weeks. A/B tested onboarding registration (email vs. Google Sign-in); Google Sign-in improved completion by 74%. Designed a 31-component Figma design system (Variables, Auto Layout, Dev Mode), used across 16 screens and implemented directly in React/TypeScript; product grew to ~280 paying customers on ~2,700 total users.",
      es: "Líder de diseño de producto para un SaaS de cumplimiento para PYMES LATAM, dueño del diseño de discovery a lanzamiento y validando el MVP en 40 días. Instrumenté Mixpanel de extremo a extremo y definí la métrica de activación; el rediseño de onboarding subió la activación de nuevos usuarios de 2.6% a 9.4% en menos de 3 semanas. A/B testé el registro de onboarding (email vs. acceso con Google); Google ganó y mejoró la completion del onboarding en 74%. Diseñé un design system de 31 componentes en Figma (Variables, Auto Layout, Dev Mode), usado en 16 pantallas e implementado en React/TypeScript; el producto creció a ~280 clientes pagando sobre ~2,700 usuarios totales."
    } },
...
const CAPS: Cap[] = [
  ...
  { label: { en: "Design Systems", es: "Design Systems" }, detail: { en: "Figma (Variables · Dev Mode · Auto Layout) · Shadcn · Radix · Storybook · WCAG 2.2 / Section 508", es: "Figma (Variables · Dev Mode · Auto Layout) · Shadcn · Radix · Storybook · WCAG 2.2 / Sección 508" } },
