...
const ROWS: Row[] = [
  { years: "2022—NOW", co: "Booz Allen Hamilton", badge: "Army · DoD · VA · FAA",
    ...
    desc: {
      en: "Led product design for the federal Trusted Environments platform and Developer Portal, architecting two design systems (Shadcn, Radix, BAH Brand Book) compliant with WCAG 2.2 and Section 508 — cutting the design-to-development cycle from ~6 weeks to under 1 week. Pioneered agentic UX research: built LLM-based persona emulators to run moderated usability testing (5 users per feature); 92% of users rated the resulting improvements better. Design system scaled organically from 0 to 26 internal teams at Booz Allen Hamilton (including Vellox Reverser cybersecurity); designed a custom MCP for developer handoff (Figma Dev Mode).",
      es: "Lideré el diseño de producto para la plataforma federal Trusted Environments y el Developer Portal, construyendo dos design systems (Shadcn, Radix, BAH Brand Book) conformes con WCAG 2.2 y Sección 508 — reduciendo el ciclo design-to-development de ~6 semanas a menos de 1 semana. Fui pionero en investigación de UX agéntica: construí emuladores de personas basados en LLM para correr pruebas de usabilidad moderadas (5 usuarios por feature); 92% calificó las mejoras como superiores. El design system escaló de 0 a 26 equipos internos en Booz Allen Hamilton (incluyendo Vellox Reverser); diseñé un MCP propio para handoff de developers."
    } },
  { years: "2026—NOW", co: "Compliance SaaS · LATAM", badge: "280+ paying users",
    ...
    desc: {
      en: "Owned discovery-to-launch design for a compliance SaaS for LATAM SMBs, shipping a validated MVP in 40 days. Drove new-user activation from 2.6% to 9.4% in under 3 weeks via onboarding redesign (Mixpanel); A/B test on registration (email vs. Google login) increased onboarding completion 74%. Built a 31-component Figma design system (Variables, Auto Layout) implemented directly in React 19 / TypeScript, scaling to ~280 paying users.",
      es: "Lideré el diseño discovery-to-launch para un SaaS de compliance para PYMES LATAM, validando el MVP en 40 días. Subí la activación de nuevos usuarios de 2.6% a 9.4% en menos de 3 semanas via rediseño del onboarding (Mixpanel); A/B test en el registro (email vs. Google) aumentó la completion en 74%. Construí un design system de 31 componentes en Figma (Variables, Auto Layout) implementado en React 19 / TypeScript, escalando a ~280 usuarios de pago."
    } },
...
const CAPS: Cap[] = [
  ...
  { label: { en: "Design Systems", es: "Design Systems" }, detail: { en: "Figma (Variables · Auto Layout · Dev Mode · Motion) · Shadcn · Radix · Storybook · WCAG 2.2 / Section 508", es: "Figma (Variables · Auto Layout · Dev Mode · Motion) · Shadcn · Radix · Storybook · WCAG 2.2 / Sección 508" } },
