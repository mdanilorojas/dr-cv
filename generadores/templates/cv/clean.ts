...
const ROWS: Row[] = [
  { years: "2022—NOW", co: "Booz Allen Hamilton", badge: "Army · DoD · VA · FAA",
    ...
    desc: {
      en: "Architected two agent-ready design systems (Shadcn, Radix, BAH Brand Book) for the federal Trusted Environments platform — WCAG 2.2 / Section 508 compliant, 95%+ automated checks passing. Cut design-to-dev cycle from ~6 weeks to under 1 week using Figma Dev Mode, Variables and Auto Layout. Pioneered agentic UX research: LLM persona emulators running 5-user moderated sessions per feature — 92% rated improvements better. Design system scaled from 0 to 26 teams at Booz Allen Hamilton (incl. Vellox Reverser cybersecurity); engineered a custom MCP for developer handoff.",
      es: "Construí dos design systems con capacidad agéntica (Shadcn, Radix, BAH Brand Book) para la plataforma federal Trusted Environments — conformes con WCAG 2.2 y Sección 508, superando el 95% de checks automatizados. Reduje el ciclo design-to-dev de ~6 semanas a menos de 1 semana usando Figma Dev Mode, Variables y Auto Layout. Pionero en investigación UX agéntica: emuladores de persona LLM corriendo sesiones moderadas de 5 usuarios por feature — 92% calificó las mejoras como mejores. Design system escaló de 0 a 26 equipos en Booz Allen Hamilton (incl. Vellox Reverser); diseñé un MCP propio para handoff."
    } },
  { years: "2026—NOW", co: "Compliance SaaS · LATAM", badge: "280+ paying users",
    ...
    desc: {
      en: "Led discovery-to-launch design for a compliance SaaS serving LATAM SMBs — validated MVP in 40 days. Instrumented Mixpanel analytics, defined the activation metric, and ran an A/B test (email vs. Google sign-in): Google won, lifting onboarding completion 74%. Rebuilt the activation flow, raising new-user activation from 2.6% to 9.4% in under 3 weeks. Built a 31-component Figma system (Variables, Auto Layout) in React 19 / TypeScript; validated flows via AI-persona loops; ~280 paying users on ~2,700 base.",
      es: "Lideré el diseño de discovery a lanzamiento de un SaaS de compliance para PYMES LATAM — MVP validado en 40 días. Instrumenté Mixpanel, definí la métrica de activación y corrí un A/B test (email vs. Google Sign-in): Google ganó, elevando la finalización del onboarding en 74%. Reconstruí el flujo de activación, subiendo la activación de nuevos usuarios de 2.6% a 9.4% en menos de 3 semanas. Construí un design system de 31 componentes en Figma (Variables, Auto Layout) en React 19 / TypeScript; validé con personas IA; ~280 usuarios pagando sobre ~2,700."
    } },
...
const CAPS: Cap[] = [
  { label: { en: "Product & UX", es: "Producto y UX" }, detail: { en: "User Research · Usability Testing · Journey Mapping · Mixpanel · A/B Testing · NPS/CSAT", es: "Investigación UX · Pruebas de Usabilidad · Journey Mapping · Mixpanel · A/B Testing · NPS/CSAT" } },
