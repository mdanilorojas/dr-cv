...
const ROWS: Row[] = [
  { years: "2022—NOW", co: "Booz Allen Hamilton", badge: "Army · DoD · VA · FAA",
    ...
    desc: {
      en: "Lead product designer for the federal Trusted Environments platform and Developer Portal, architecting two design systems (Shadcn, Radix, BAH Brand Book) compliant with WCAG 2.2 and Section 508 — passing 95%+ of automated accessibility checks. Pioneered agentic UX research: built LLM-based persona emulators to run moderated usability testing (5 sessions per feature); 92% of participants rated resulting improvements better. Cut design-to-dev cycle from ~6 weeks to under 1 week via Figma (Auto Layout, Variables, Dev Mode) and production-ready React handoff. Designed a custom MCP for developers; design system scaled from 0 to 26 internal teams, including the Vellox Reverser cybersecurity product.",
      es: "Diseñador de producto principal para la plataforma federal Trusted Environments y el Developer Portal, construyendo dos design systems (Shadcn, Radix, BAH Brand Book) conformes con WCAG 2.2 y Sección 508 — superando el 95% de checks automatizados de accesibilidad. Fui pionero en investigación de UX agéntica: construí emuladores de personas basados en LLM para correr pruebas de usabilidad moderadas (5 sesiones por feature); 92% de participantes aprobaron las mejoras resultantes. Reduje el ciclo diseño-a-dev de ~6 semanas a menos de 1 semana via Figma (Auto Layout, Variables, Dev Mode) y entrega de React lista para producción. Diseñé un MCP propio para developers; el design system escaló de 0 a 26 equipos internos, incluyendo el producto de ciberseguridad Vellox Reverser."
    } },
  { years: "2026—NOW", co: "Compliance SaaS · LATAM", badge: "280+ paying users",
    ...
    desc: {
      en: "Lead product designer for a compliance SaaS for LATAM SMBs, owning discovery-to-launch design and shipping a validated MVP in 40 days. Instrumented product analytics in Mixpanel, defining the activation metric; onboarding redesign raised new-user activation from 2.6% to 9.4% in under 3 weeks. Ran an A/B test (email vs. Google OAuth) improving onboarding completion 74%. Designed a 31-component Figma design system implemented in React/TypeScript with AI-persona feedback loops to validate flows pre-engineering.",
      es: "Líder de diseño de producto para un SaaS de cumplimiento para PYMES LATAM, dueño del diseño de discovery a lanzamiento y validando el MVP en 40 días. Instrumenté analíticas en Mixpanel, definiendo la métrica de activación; el rediseño de onboarding subió la activación de nuevos usuarios de 2.6% a 9.4% en menos de 3 semanas. A/B test (email vs. Google OAuth): Google ganó, mejorando la finalización de onboarding un 74%. Diseñé un design system de 31 componentes en Figma implementado en React/TypeScript con loops de feedback con personas IA."
    } },
...
const CAPS: Cap[] = [
  ...
  { label: { en: "Agentic UX / AI", es: "UX Agéntica / IA" }, detail: { en: "Agent-ready Design Systems · LLM Persona Emulation · Custom MCP Design · AI-augmented Workflows", es: "Design Systems Agénticos · Emulación de Personas LLM · MCP a Medida · Flujos Aumentados con IA" } },
];
